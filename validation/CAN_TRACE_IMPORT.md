...
Every imported frame preserves:

- timestamp in integer nanoseconds
- timestamp provenance
- source format
- channel/interface
- receive/transmit direction when the source exposes it
- arbitration ID
- standard/extended-ID classification
- DLC
- exact payload bytes as uppercase hexadecimal

Capture-level provenance also records:
...
`null` is intentional: the importer never assumes an adapter was listen-only merely because the file contains received frames.
...
Hardware/driver characterization must therefore occur before timestamps are used for tight cross-source agreement.

Non-monotonic timestamps within one imported stream are rejected rather than silently reordered.
